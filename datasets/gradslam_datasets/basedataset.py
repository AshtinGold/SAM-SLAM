...
from utils.keyframe_selection import find_unique_class_rows
...
class GradSLAMDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        self.color_paths, self.depth_paths, self.embedding_paths, self.s_paths, self.f_paths = self.get_filepaths()

        if len(self.color_paths) != len(self.depth_paths):
            raise ValueError("Number of color and depth images must be the same.")
        if len(self.color_paths) != len(self.s_paths):
            raise ValueError("Number of color and semantic images must be the same.")
        if len(self.color_paths) != len(self.f_paths):
            raise ValueError("Number of color and semantic images must be the same.")
        ...
        self.s_paths =  self.s_paths[self.start : self.end : stride] #>>
        self.f_paths =  self.f_paths[self.start : self.end : stride] #>>
    ...
    # >>
    def get_language_feature(self, seg, feat):
        """
        ...
        height, width = self.desired_height, self.desired_width
        ...
        seg = seg_map[:, y, x].squeeze(-1).astype(np.int64)
        mask = seg != -1  # >> change to 0 later

        all_lvl_feats = np.zeros((n_seg_lvls, le_dim, height, width))
        all_lvl_masks = np.zeros((n_seg_lvls, 1, height, width))

        for i in range(1, 4):  # only use s, m, l
            point_feature1 = feature_map[seg[i:i+1]].squeeze(0)
            point_feature = point_feature1.reshape(height, width, -1).transpose(2, 0, 1)
            all_lvl_feats[i-1] = point_feature

            lvl_mask = mask[i:i+1].reshape(1, height, width)
            all_lvl_masks[i-1] = lvl_mask

        reshape_factor = le_dim * n_seg_lvls
        all_lvl_feats = all_lvl_feats.reshape(reshape_factor, height, width)  # 9 x H X W
        all_lvl_masks = all_lvl_masks.reshape(n_seg_lvls, height, width)  # 3 x H x W

        # all_lvl_feats and all_lvl_masks now contain the desired numpy arrays
        return all_lvl_feats, all_lvl_masks

    def _preprocess_le(self, all_le):
    ...
    def __getitem__(self, index):
        ...
        # >>
        feature_map = np.load(f_path)  # 150 x 3
        seg_map = np.load(s_path)  # 4 X 680 X 1200  ['default', 's','m','l']
        seg_map = self._preprocess_le(seg_map)

        pointwise_le, le_masks = self.get_language_feature(seg_map, feature_map) # 9 x h x w / 3 x h x w
        ...
        pointwise_le = torch.from_numpy(pointwise_le) #>>
        le_masks = torch.from_numpy(le_masks) #>>
        K = torch.from_numpy(K)
        ...
        if self.load_embeddings:
            ...
            return (
                ...
                pointwise_le.to(self.device).type(self.dtype),
                le_masks.to(self.device).type(self.dtype),
