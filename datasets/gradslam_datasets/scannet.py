...
class ScannetDataset(GradSLAMDataset):
    ...
    def get_filepaths(self):
        ...
        sem_paths = natsorted(glob.glob(f"{self.input_folder}/sem-maps/*.png"))
        lbs_paths = natsorted(glob.glob(f"{self.input_folder}/label-filt/*.png")) ## ap08
        embedding_paths = None
        if self.load_embeddings:
            embedding_paths = natsorted(glob.glob(f"{self.input_folder}/{self.embedding_dir}/*.pt"))
        return color_paths, depth_paths, embedding_paths, sem_paths, lbs_paths
