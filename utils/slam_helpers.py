...
def params2rendervar(params): ## never actually used in code
...
#>>
def transformed_params2rendervar(params, feat_lvl, transformed_gaussians):
    # Check if Gaussians are Isotropic
    if params['log_scales'].shape[1] == 1:
        log_scales = torch.tile(params['log_scales'], (1, 3))
    else:
    ...
    #>>
    if feat_lvl == 0: # s
        language_feature_precomp = params['pointwise_le_s']
    if feat_lvl == 1: # m
        language_feature_precomp = params['pointwise_le_m']
    if feat_lvl == 2: # l
        language_feature_precomp = params['pointwise_le_l']
...
#     # Initialize Render Variables
#     sil_color = torch.zeros_like(params['rgb_colors'])
#     sil_color[:, 0] = 1.0   ## make red channel 1.0, red and green both 0.0
    
#     language_feature_precomp = torch.zeros((1,), device="cuda") #>> No LE
...
def transformed_params2depthplussilhouette(params, w2c, transformed_gaussians):
    ...
    N = params['logit_opacities'].shape[0] # 
    language_feature_precomp = torch.zeros(N, 3, device='cuda')
