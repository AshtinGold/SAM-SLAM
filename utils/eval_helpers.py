...
def plot_rgbd_silhouette(color, depth, le_all, rastered_color, rastered_depth, rastered_le_all, presence_sil_mask, diff_depth_l1,
    ...
    fig, axs = plt.subplots(2, 6, figsize=(fig_width, fig_height))
    ...
    axs[1, 0].set_title("Rasterized RGB, PSNR: {:.2f}".format(psnr), fontsize = 8.5)
    axs[1, 1].imshow(rastered_depth[0, :, :].cpu(), cmap='jet', vmin=0, vmax=6)
    axs[1, 1].set_title("Rasterized Depth, L1: {:.2f}".format(depth_l1), fontsize = 8.5)
    ...
    # >>
    # titles = ['lv1', 'lv2', 'lv3']
    print('rastered_le_all.shape')
    print(rastered_le_all.shape)
    print('le_all.shape')
    print(le_all.shape)

    for i in range(3):
        print(rastered_le_all[3*i:3+3*i].shape)
        print('i')
        print(i)
        axs[0, 3 + i].imshow(rastered_le_all[3*i:3+3*i].cpu().permute(1, 2, 0), cmap="jet")
        # axs[0, 3 + i].set_title(f"Rastered {titles[i]} rendering")
        # Display the ground truth image with PSNR
        axs[1, 3 + i].imshow(le_all[3*i:3+3*i].cpu().permute(1, 2, 0))
        # axs[1, 3 + i].set_title(f"Ground truth {titles[i]} LE, PSNR: {{:.2f}}".format(globals()[f'psnr_{titles[i]}']))
...
def report_progress(params, data, i, progress_bar, iter_time_idx, sil_thres, every_i=1, qual_every_i=1, 
    ...
    if i % every_i == 0 or i == 1:
        ...
        rendervar1 = transformed_params2rendervar(params, 0, transformed_gaussians)
        im, all_lvl_renders, radius = Renderer(raster_settings=data['cam'])(**rendervar1)
        print('all lvl shape') # ERR: torch.Size([3, 340, 600])
        print()
        print(all_lvl_renders.shape)
        raise
        ...
        if tracking:
            ...
            lv1_psnr = calc_psnr(lv1_le_render * presence_sil_mask, data['pointwise_le'][0:3] * presence_sil_mask).mean()
            lv2_psnr = calc_psnr(lv2_le_render * presence_sil_mask, data['pointwise_le'][3:6] * presence_sil_mask).mean()
            lv3_psnr = calc_psnr(lv3_le_render * presence_sil_mask, data['pointwise_le'][6:9] * presence_sil_mask).mean()
        else:
            psnr = calc_psnr(im, data['im']).mean()
            lv1_psnr = calc_psnr(lv1_le_render, data['pointwise_le'][0:3]).mean()
            lv2_psnr = calc_psnr(lv2_le_render, data['pointwise_le'][3:6]).mean()
            lv3_psnr = calc_psnr(lv3_le_render, data['pointwise_le'][6:9]).mean()
        ...
        if wandb_run is not None:
            wandb_log = {f"{stage}/PSNR": psnr,
                        f"{stage}/Le_PSNR_lv1": lv1_psnr,
                        f"{stage}/Le_PSNR_lv2": lv2_psnr,
                        f"{stage}/Le_PSNR_lv3": lv3_psnr,
        ...
        if wandb_save_qual and (i % qual_every_i == 0 or i == 1):
            ...
            all_le_renders = torch.cat([lv1_le_render, lv2_le_render, lv3_le_render], dim=0)
            plot_rgbd_silhouette(data['im'], data['depth'], data['pointwise_le'], im, rastered_depth, all_le_renders, presence_sil_mask, diff_depth_l1,
...
def eval_online(dataset, all_params, num_frames, eval_online_dir, sil_thres,
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        color, depth, pointwise_le, le_maps, intrinsics, pose = dataset[time_idx]
        ...
        curr_data = {'cam': cam, 'im': color, 'depth': depth, 'id': time_idx, 'intrinsics': intrinsics, 'w2c': first_frame_w2c, 'pointwise_le': pointwise_le, 'le_maps': le_maps}
        ...
        # Initialize Render Variables
        rendervar = transformed_params2rendervar(params, 2, transformed_gaussians)
        ...
        im, _, radius = Renderer(raster_settings=curr_data['cam'])(**rendervar)
...
def eval(dataset, final_params, num_frames, eval_dir, sil_thres, 
    ...
    if save_frames:
        ...
        render_le_dir = os.path.join(eval_dir, "rendered_le") #>> rendered
        os.makedirs(render_le_dir, exist_ok=True)
        pointwise_le_dir = os.path.join(eval_dir, "pointwise_le") #>> gt
        os.makedirs(pointwise_le_dir, exist_ok=True)
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        # Render RGB and Calculate PSNR
        # region
        rendervar1 = transformed_params2rendervar(final_params, 0, transformed_gaussians)
        im, lv1_le_render, radius = Renderer(raster_settings=curr_data['cam'])(**rendervar1)

        rendervar2 = transformed_params2rendervar(final_params, 1, transformed_gaussians)
        _, lv2_le_render, _ = Renderer(raster_settings=curr_data['cam'])(**rendervar2)

        rendervar3 = transformed_params2rendervar(final_params, 2, transformed_gaussians)
        _, lv3_le_render, _ = Renderer(raster_settings=curr_data['cam'])(**rendervar3)
        # endregion
        ...
        #>> LE losses
        #>> if mapping_iters==0 and not add_new_gaussians:
        weighted_lv1_le = lv1_le_render * presence_sil_mask * valid_depth_mask
        weighted_gt_lv1_le = curr_data['pointwise_le'][0:3] * presence_sil_mask * valid_depth_mask
        
        weighted_lv2_le = lv2_le_render * presence_sil_mask * valid_depth_mask
        weighted_gt_lv2_le = curr_data['pointwise_le'][3:6] * presence_sil_mask * valid_depth_mask
        
        weighted_lv3_le = lv3_le_render * presence_sil_mask * valid_depth_mask
        weighted_gt_lv3_le = curr_data['pointwise_le'][6:9] * presence_sil_mask * valid_depth_mask
    else:
        weighted_lv1_le = lv1_le_render * valid_depth_mask
        weighted_gt_lv1_le = curr_data['pointwise_le'][0:3] * valid_depth_mask
        
        weighted_lv2_le = lv2_le_render * valid_depth_mask
        weighted_gt_lv2_le = curr_data['pointwise_le'][3:6] * valid_depth_mask
        
        weighted_lv3_le = lv3_le_render * valid_depth_mask
        weighted_gt_lv3_le = curr_data['pointwise_le'][6:9] * valid_depth_mask


        # Calculate metrics for lv1
        lv1_psnr = calc_psnr(weighted_lv1_le, weighted_gt_lv1_le).mean()
        lv1_ssim = ms_ssim(weighted_lv1_le.unsqueeze(0).cpu(), weighted_gt_lv1_le.unsqueeze(0).cpu(), data_range=1.0, size_average=True)
        lv1_lpips_score = loss_fn_alex(torch.clamp(weighted_lv1_le.unsqueeze(0), 0.0, 1.0), torch.clamp(weighted_gt_lv1_le.unsqueeze(0), 0.0, 1.0)).item()
        lv1_psnr_list.append(lv1_psnr.cpu().numpy())
        lv1_ssim_list.append(lv1_ssim.cpu().numpy())
        lv1_lpips_list.append(lv1_lpips_score)

        # Calculate metrics for lv2
        lv2_psnr = calc_psnr(weighted_lv2_le, weighted_gt_lv2_le).mean()
        lv2_ssim = ms_ssim(weighted_lv2_le.unsqueeze(0).cpu(), weighted_gt_lv2_le.unsqueeze(0).cpu(), data_range=1.0, size_average=True)
        lv2_lpips_score = loss_fn_alex(torch.clamp(weighted_lv2_le.unsqueeze(0), 0.0, 1.0), torch.clamp(weighted_gt_lv2_le.unsqueeze(0), 0.0, 1.0)).item()
        lv2_psnr_list.append(lv2_psnr.cpu().numpy())
        lv2_ssim_list.append(lv2_ssim.cpu().numpy())
        lv2_lpips_list.append(lv2_lpips_score)

        # Calculate metrics for lv3
        lv3_psnr = calc_psnr(weighted_lv3_le, weighted_gt_lv3_le).mean()
        lv3_ssim = ms_ssim(weighted_lv3_le.unsqueeze(0).cpu(), weighted_gt_lv3_le.unsqueeze(0).cpu(), data_range=1.0, size_average=True)
        lv3_lpips_score = loss_fn_alex(torch.clamp(weighted_lv3_le.unsqueeze(0), 0.0, 1.0), torch.clamp(weighted_gt_lv3_le.unsqueeze(0), 0.0, 1.0)).item()
        lv3_psnr_list.append(lv3_psnr.cpu().numpy())
        lv3_ssim_list.append(lv3_ssim.cpu().numpy())
        lv3_lpips_list.append(lv3_lpips_score)
        ...
        if save_frames:  #>> todo? I never see this being saved
            ...
            cv2.imwrite(os.path.join(render_le_dir, "gs_lv1_{:04d}.png".format(time_idx)), cv2.cvtColor(lv1_le_render*255, cv2.COLOR_RGB2BGR))
            cv2.imwrite(os.path.join(render_le_dir, "gs_lv2_{:04d}.png".format(time_idx)), cv2.cvtColor(lv2_le_render*255, cv2.COLOR_RGB2BGR))
            cv2.imwrite(os.path.join(render_le_dir, "gs_lv3_{:04d}.png".format(time_idx)), cv2.cvtColor(lv3_le_render*255, cv2.COLOR_RGB2BGR))
            #>> TO-DO - do it for other render levels
        ...
        all_le_renders = torch.cat([lv1_le_render, lv2_le_render, lv3_le_render], dim=0)
        if wandb_run is None:
            plot_rgbd_silhouette(color, depth, pointwise_le, im, rastered_depth_viz, all_le_renders, presence_sil_mask, diff_depth_l1,
                                 psnr, depth_l1, fig_title, plot_dir, 
                                 plot_name=plot_name, save_plot=True)
        elif wandb_save_qual:
            plot_rgbd_silhouette(color, depth, pointwise_le, im, rastered_depth_viz, all_le_renders, presence_sil_mask, diff_depth_l1,
    ...
    #>>
    lv1_psnr_list = np.array(lv1_psnr_list)
    lv1_ssim_list = np.array(lv1_ssim_list)
    lv1_lpips_list = np.array(lv1_lpips_list)

    lv2_psnr_list = np.array(lv2_psnr_list)
    lv2_ssim_list = np.array(lv2_ssim_list)
    lv2_lpips_list = np.array(lv2_lpips_list)

    lv3_psnr_list = np.array(lv3_psnr_list)
    lv3_ssim_list = np.array(lv3_ssim_list)
    lv3_lpips_list = np.array(lv3_lpips_list)
    ...
    #>>
    avg_lv1_psnr = lv1_psnr_list.mean()
    avg_lv1_ssim = lv1_ssim_list.mean()
    avg_lv1_lpips = lv1_lpips_list.mean()

    avg_lv2_psnr = lv2_psnr_list.mean()
    avg_lv2_ssim = lv2_ssim_list.mean()
    avg_lv2_lpips = lv2_lpips_list.mean()

    avg_lv3_psnr = lv3_psnr_list.mean()
    avg_lv3_ssim = lv3_ssim_list.mean()
    avg_lv3_lpips = lv3_lpips_list.mean()

    avg_le_psnr = np.array([avg_lv1_psnr, avg_lv2_psnr, avg_lv3_psnr]).mean()
    avg_le_ssim = np.array([avg_lv1_ssim, avg_lv2_ssim, avg_lv3_ssim]).mean()
    avg_le_lpips = np.array([avg_lv1_lpips, avg_lv2_lpips, avg_lv3_lpips]).mean()
    ...
    print("Average LE PSNR: {:.2f}".format(avg_le_psnr))
    print("Average LE SSIM: {:.3f}".format(avg_le_ssim))
    print("Average LE LPIPS: {:.3f}".format(avg_le_lpips))
    ...
    if wandb_run is not None:
        wandb_run.log({"Final Stats/Average PSNR": avg_psnr,
                    ...
                    "Final Stats/Average LE PSNR": avg_le_psnr,
                    "Final Stats/Average LE SSIM": avg_le_ssim,
                    "Final Stats/Average LE LPIPS": avg_le_lpips,
                    "Final Stats/step": 1})
    ...
    print("Average LE PSNR (lv1, lv2, lv3): {:.2f}".format(avg_le_psnr))
    print("Average LE SSIM (lv1, lv2, lv3): {:.3f}".format(avg_le_ssim))
    print("Average LE LPIPS (lv1, lv2, lv3): {:.3f}".format(avg_le_lpips))
    ...
    #>>
    axs[1, 0].plot(np.arange(len(lv1_psnr_list)), lv1_psnr_list)
    axs[1, 0].set_title("LE LV1 PSNR")
    axs[1, 0].set_xlabel("Time Step")
    axs[1, 0].set_ylabel("PSNR")
    axs[1, 1].plot(np.arange(len(lv1_ssim_list)), lv1_ssim_list)
    axs[1, 1].set_title("LE LV2 SSIM")
    axs[1, 1].set_xlabel("Time Step")
    axs[1, 1].set_ylabel("SSIM")

    fig.suptitle("Average PSNR: {:.2f}, Average Depth L1: {:.2f} cm, Average RMSE: {:.2f} cm, Average LE PSNR: {:.2f}, Average LE SSIM: {:.3f}".format(avg_psnr, avg_l1 * 100, avg_rmse * 100, avg_le_psnr, avg_le_ssim), y=1.05, fontsize=16)
    plt.tight_layout()
...
# eval_nvs is unused in splatam.py ... Authors probably used it independently in debugging
