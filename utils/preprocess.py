...
def create(image_list, data_list, save_folder):
    ...
    total_lengths = []
    timer = 0
    img_embeds = torch.zeros((len(image_list), 255, embed_size)) # CRITICAL ** - Map indices [-1, 254] to [0 to 255]. this gives 255 possible classes, remapping -1 to 0.
    seg_maps = torch.zeros((len(image_list), 4, *image_list[0].shape[1:])) 
    # *image_list[0].shape[1:]: The height and width of the images, which are extracted from the shape of the first image in image_list.
    mask_generator.predictor.model.to('cuda')

    for i, img in tqdm(enumerate(image_list), desc="Embedding images", leave=False):
        timer += 1
        try:  ## for each single image
            img_embed, seg_map = _embed_clip_sam_tiles(img.unsqueeze(0), sam_encoder)
        except:
            raise ValueError(timer)

        lengths = [len(v) for k, v in img_embed.items()]  # k : 'default','s','m','l'
        total_length = sum(lengths)  # counts total number of image embeddings.
        total_lengths.append(total_length)
        
        # extend second dimension of 'img_embeds' to 'total_length' in cases where # classes > 300.
        if total_length > img_embeds.shape[1]:
            pad = total_length - img_embeds.shape[1]
            img_embeds = torch.cat([
                img_embeds,
                torch.zeros((len(image_list), pad, embed_size))  # pad with 0s, extending the dimension of number of classes.
            ], dim=1)

        img_embed = torch.cat([v for k, v in img_embed.items()], dim=0)
        assert img_embed.shape[0] == total_length
        img_embeds[i, :total_length] = img_embed  # dim2,3 is fitted with img_embed
        
        seg_map_tensor = []
        lengths_cumsum = lengths.copy()
        for j in range(1, len(lengths)):
            lengths_cumsum[j] += lengths_cumsum[j-1]
        # postprocessing to align indices of multiple segmentation maps with varying segment lengths, ensuring consistent indexing.
    ...
    for i in range(img_embeds.shape[0]):
        ...
        assert total_lengths[i] == int(seg_maps[i].max() + 1)  # total length should equal the largest pixel label number in seg-map.
        curr = {
            'feature': img_embeds[i, :total_lengths[i]],
            'seg_maps': seg_maps[i]
        }
        sava_numpy(save_path, curr)  # huge memory requirement! 

def sava_numpy(save_path, data):
    save_path_s = save_path + '_s.npy' # seg maps
    save_path_f = save_path + '_f.npy' # features
...
def sam_encoder(image):
    ...
    def mask2segmap(masks, image):
        seg_img_list = []
        seg_map = np.zeros(image.shape[:2], dtype=np.uint8) # >> change from int32 to uint8
        masks = masks[:255]   # >> TRUNCATE off 255 classes
        for i in range(len(masks)):
            ...
            seg_map[masks[i]['segmentation']] = i+1  # remap indices to save space
