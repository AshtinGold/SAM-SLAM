...
from utils.slam_helpers import (
    transform_to_frame, transformed_params2rendervar, transformed_params2depthplussilhouette, transformed_params2semanticvar,
...
## SGS -- BEGIN
def plot_rgbdsem_silhouette(color, depth, rastered_color, rastered_depth, presence_sil_mask, diff_depth_l1,
                            sem_color, rastered_sem_color,
                         psnr, depth_l1, fig_title, plot_dir=None, plot_name=None, 
                         save_plot=False, wandb_run=None, wandb_step=None, wandb_title=None, diff_rgb=None):
    # Determine Plot Aspect Ratio
    aspect_ratio = color.shape[2] / color.shape[1]
    ...
    axs[1, 2].set_title("Diff Depth L1")
    ## #ap1802
    ## :: begin ADDING SEM COLOR
    axs[0, 3].imshow(sem_color.cpu().permute(1, 2, 0))
    axs[0, 3].set_title("Ground Truth sem RGB")
    rastered_sem_color = torch.clamp(rastered_sem_color, 0, 1)
    axs[1, 3].imshow(rastered_sem_color.cpu().permute(1, 2, 0))
    axs[1, 3].set_title("Rasterized sem RGB")
    ## -- end
    ##
    for ax in axs.flatten():
        ax.axis('off')
...
def report_progress(params, data, i, progress_bar, iter_time_idx, sil_thres, every_i=1, qual_every_i=1, 
    ...
    if i % every_i == 0 or i == 1:
        ...
        semanticvar = transformed_params2semanticvar(params, transformed_gaussians) 
        ...
        sem_im, _,  _, = Renderer(raster_settings=data['cam'])(**semanticvar)
        ...
        if wandb_save_qual and (i % qual_every_i == 0 or i == 1):
            ...
            plot_rgbdsem_silhouette(data['im'], data['depth'], im, rastered_depth, presence_sil_mask, diff_depth_l1, data['sem_im'], sem_im, # ap18
                                 psnr, depth_l1, fig_title, wandb_run=wandb_run, wandb_step=wandb_step, 
                                 wandb_title=f"{stage} Qual Viz")
...
def eval_online(dataset, all_params, num_frames, eval_online_dir, sil_thres,
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        color, depth, sem_color, intrinsics, pose, _ = dataset[time_idx]
        intrinsics = intrinsics[:3, :3]

        # Process RGB-D Data
        color = color.permute(2, 0, 1) / 255 # (H, W, C) -> (C, H, W)
        sem_color = sem_color.permute(2, 0, 1) / 255 # (H, W, C) -> (C, H, W)
        ...
        # Render sem map
        sem_im, _,  _, = Renderer(raster_settings=curr_data['cam'])(**semanticvar)
        ...
        if wandb_run is None:
            plot_rgbdsem_silhouette(color, depth, im, rastered_depth, presence_sil_mask, diff_depth_l1,
                                 sem_color, sem_im, psnr, depth_l1, fig_title, plot_dir, 
...
def eval(dataset, final_params, num_frames, eval_dir, sil_thres, 
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        color = color.permute(2, 0, 1) / 255 # (H, W, C) -> (C, H, W)
        depth = depth.permute(2, 0, 1) # (H, W, C) -> (C, H, W)
        sem_color = sem_color.permute(2, 0, 1) / 255
        ...
        # Define current frame data
        curr_data = {'cam': cam, 'im': color, 'depth': depth, 'id': time_idx, 'intrinsics': intrinsics, 'w2c': first_frame_w2c, 'sem_im': sem_color}

        # Initialize Render Variables
        rendervar = transformed_params2rendervar(final_params, transformed_gaussians)
        semanticvar = transformed_params2semanticvar(final_params, transformed_gaussians) 
        ...
        presence_sil_mask = presence_sil_mask.detach().cpu().numpy()
        if wandb_run is None:
            plot_rgbdsem_silhouette(color, depth, im, rastered_depth_viz, presence_sil_mask, diff_depth_l1, # im is rasterized
                                sem_color, sem_im, # sem_im is rasterized
                                 psnr, depth_l1, fig_title, plot_dir, 
                                 plot_name=plot_name, save_plot=True)
        elif wandb_save_qual:
            plot_rgbdsem_silhouette(color, depth, im, rastered_depth_viz, presence_sil_mask, diff_depth_l1,
                                 sem_color, sem_im, 
                                 psnr, depth_l1, fig_title, plot_dir, 
                                 plot_name=plot_name, save_plot=True,
                                 wandb_run=wandb_run, wandb_step=None, 
