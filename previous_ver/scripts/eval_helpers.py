...
def plot_rgbd_silhouette(color, depth, le_l, rastered_color, rastered_depth,    rastered_le_l, presence_sil_mask, diff_depth_l1,
    ...
    #
    axs[0, 3].imshow(rastered_le_l.cpu().permute(1, 2, 0), cmap="jet")
    axs[0, 3].set_title("Rastered L LE rendering")
    axs[1, 3].imshow(le_l.cpu().permute(1, 2, 0))
    axs[1, 3].set_title("Ground truth L LE, PSNR: {:.2f}".format(psnr))
...
def report_progress(params, data, i, progress_bar, iter_time_idx, sil_thres, every_i=1, qual_every_i=1, 
    ...
    if i % every_i == 0 or i == 1:
        ...
        im, lv3_le_render, _, = Renderer(raster_settings=data['cam'])(**rendervar)
        if tracking:
            psnr = calc_psnr(im * presence_sil_mask, data['im'] * presence_sil_mask).mean()
            le_psnr = calc_psnr(lv3_le_render * presence_sil_mask, data['pointwise_le'][6:9] * presence_sil_mask).mean()
        else:
            psnr = calc_psnr(im, data['im']).mean()
            le_psnr = calc_psnr(lv3_le_render, data['pointwise_le'][6:9]).mean()
        ...
        if wandb_save_qual and (i % qual_every_i == 0 or i == 1):
            ...
            plot_rgbd_silhouette(data['im'], data['depth'], data['pointwise_le'][6:9], im, rastered_depth, lv3_le_render, presence_sil_mask, diff_depth_l1,
...
def eval(dataset, final_params, num_frames, eval_dir, sil_thres, 
    ...
    ssim_list = []
    #>>
    le_psnr_list, le_lpips_list, le_ssim_list = list(), list(), list()
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        # Initialize Render Variables
        rendervar = transformed_params2rendervar(final_params, 2, transformed_gaussians)
        ...
        # Render RGB and Calculate PSNR
        im, lv3_le_render, _, = Renderer(raster_settings=curr_data['cam'])(**rendervar)
        if mapping_iters==0 and not add_new_gaussians:
        ...
        #>> Render lvl-3 Semantic RGB -- honestly I dont understand the criteria for calculating qualitatitive scores differently in each function.
        # region
        if mapping_iters==0 and not add_new_gaussians:
            weighted_l_le = lv3_le_render * presence_sil_mask * valid_depth_mask
            weighted_gt_l_le = curr_data['pointwise_le'][6:9] * presence_sil_mask * valid_depth_mask
        else:
            weighted_l_le = lv3_le_render * valid_depth_mask
            weighted_gt_l_le = curr_data['pointwise_le'][6:9] * valid_depth_mask
            
        le_psnr = calc_psnr(weighted_l_le, weighted_gt_l_le).mean()
        le_ssim = ms_ssim(weighted_l_le.unsqueeze(0).cpu(), weighted_gt_l_le.unsqueeze(0).cpu(), 
                        data_range=1.0, size_average=True)
        le_lpips_score = loss_fn_alex(torch.clamp(weighted_l_le.unsqueeze(0), 0.0, 1.0),
                                    torch.clamp(weighted_gt_l_le.unsqueeze(0), 0.0, 1.0)).item()

        le_psnr_list.append(le_psnr.cpu().numpy())
        le_ssim_list.append(le_ssim.cpu().numpy())
        le_lpips_list.append(le_lpips_score)
        ...
        if save_frames:  #>> todo? I never see this being saved
            ...
            cv2.imwrite(os.path.join(render_le_dir, "gs_{:04d}.png".format(time_idx)), cv2.cvtColor(viz_render_im*255, cv2.COLOR_RGB2BGR))
        ...
        presence_sil_mask = presence_sil_mask.detach().cpu().numpy()
        if wandb_run is None:
            plot_rgbd_silhouette(color, depth, pointwise_le[6:9], im, rastered_depth_viz, lv3_le_render, presence_sil_mask, diff_depth_l1,
                                 psnr, depth_l1, fig_title, plot_dir, 
                                 plot_name=plot_name, save_plot=True)
        elif wandb_save_qual:
            plot_rgbd_silhouette(color, depth, pointwise_le[6:9], im, rastered_depth_viz, lv3_le_render, presence_sil_mask, diff_depth_l1,
    ...
    #>>
    le_psnr_list = np.array(le_psnr_list)
    le_ssim_list = np.array(le_ssim_list)
    le_lpips_list = np.array(le_lpips_list)
    ...
    #>>
    avg_le_psnr = le_psnr_list.mean()
    avg_le_ssim = le_ssim_list.mean()
    avg_le_lpips = le_lpips_list.mean()
    ...
    np.savetxt(os.path.join(eval_dir, "le_psnr.txt"), le_psnr_list)
    np.savetxt(os.path.join(eval_dir, "le_ssim.txt"), le_ssim_list)
    np.savetxt(os.path.join(eval_dir, "le_lpips.txt"), le_lpips_list)
    ...
    #>>
    axs[1, 0].plot(np.arange(len(le_psnr_list)), le_psnr_list)
    axs[1, 0].set_title("LE PSNR")
    axs[1, 0].set_xlabel("Time Step")
    axs[1, 0].set_ylabel("PSNR")
    axs[1, 1].plot(np.arange(len(le_ssim_list)), le_ssim_list)
    axs[1, 1].set_title("LE SSIM")
